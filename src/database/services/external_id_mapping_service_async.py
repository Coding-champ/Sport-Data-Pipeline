...
import asyncpg
from src.common.constants import Source, normalize_source
...
class ExternalIdMappingServiceAsync:
    ...
    async def ensure(
        ...
        *,
        ...
        async with conn.transaction():
            status = await conn.execute(insert_sql, source_norm, external_id, internal_id)
            # asyncpg Status wie "INSERT 0 1" bei Erfolg
            if status.endswith("1"):
                return internal_id

            row = await conn.fetchrow(select_sql, source_norm, external_id)
            if row is None:
    ...
    # Bequeme Wrapper
    async def ensure_player(self, conn, *, source: str | Source, external_id: str, player_id: int) -> int:
        return await self.ensure(
            conn, entity="player", source=source, external_id=external_id, internal_id=player_id
        )

    async def find_player(self, conn, *, source: str | Source, external_id: str) -> Optional[int]:
        return await self.find(conn, entity="player", source=source, external_id=external_id)

    async def ensure_team(self, conn, *, source: str | Source, external_id: str, team_id: int) -> int:
        return await self.ensure(
            conn, entity="team", source=source, external_id=external_id, internal_id=team_id
        )

    async def find_team(self, conn, *, source: str | Source, external_id: str) -> Optional[int]:
        return await self.find(conn, entity="team", source=source, external_id=external_id)

    async def ensure_club(self, conn, *, source: str | Source, external_id: str, club_id: int) -> int:
        return await self.ensure(
            conn, entity="club", source=source, external_id=external_id, internal_id=club_id
        )

    async def find_club(self, conn, *, source: str | Source, external_id: str) -> Optional[int]:
        return await self.find(conn, entity="club", source=source, external_id=external_id)

    async def ensure_match(self, conn, *, source: str | Source, external_id: str, match_id: int) -> int:
        return await self.ensure(
            conn, entity="match", source=source, external_id=external_id, internal_id=match_id
        )

    async def find_match(self, conn, *, source: str | Source, external_id: str) -> Optional[int]:
        return await self.find(conn, entity="match", source=source, external_id=external_id)

    async def ensure_league(self, conn, *, source: str | Source, external_id: str, league_id: int) -> int:
        return await self.ensure(
            conn, entity="league", source=source, external_id=external_id, internal_id=league_id
        )

    async def find_league(self, conn, *, source: str | Source, external_id: str) -> Optional[int]:
        return await self.find(conn, entity="league", source=source, external_id=external_id)

    async def ensure_competition(
        self, conn, *, source: str | Source, external_id: str, competition_id: int
    ) -> int:
        return await self.ensure(
            conn,
            entity="competition",
    ...
    async def find_competition(self, conn, *, source: str | Source, external_id: str) -> Optional[int]:
        return await self.find(conn, entity="competition", source=source, external_id=external_id)

    async def ensure_tournament(
        self, conn, *, source: str | Source, external_id: str, tournament_id: int
    ) -> int:
        return await self.ensure(
            conn,
            entity="tournament",
    ...
    async def find_tournament(self, conn, *, source: str | Source, external_id: str) -> Optional[int]:
        return await self.find(conn, entity="tournament", source=source, external_id=external_id)

    async def ensure_venue(self, conn, *, source: str | Source, external_id: str, venue_id: int) -> int:
        return await self.ensure(
            conn, entity="venue", source=source, external_id=external_id, internal_id=venue_id
        )

    async def find_venue(self, conn, *, source: str | Source, external_id: str) -> Optional[int]:
        return await self.find(conn, entity="venue", source=source, external_id=external_id)

    async def ensure_stadium(self, conn, *, source: str | Source, external_id: str, stadium_id: int) -> int:
        return await self.ensure(
            conn, entity="stadium", source=source, external_id=external_id, internal_id=stadium_id
        )

    async def find_stadium(self, conn, *, source: str | Source, external_id: str) -> Optional[int]:
        return await self.find(conn, entity="stadium", source=source, external_id=external_id)

    async def ensure_referee(self, conn, *, source: str | Source, external_id: str, referee_id: int) -> int:
        return await self.ensure(
            conn, entity="referee", source=source, external_id=external_id, internal_id=referee_id
        )

    async def find_referee(self, conn, *, source: str | Source, external_id: str) -> Optional[int]:
        return await self.find(conn, entity="referee", source=source, external_id=external_id)

    async def ensure_coach(self, conn, *, source: str | Source, external_id: str, coach_id: int) -> int:
        return await self.ensure(
            conn, entity="coach", source=source, external_id=external_id, internal_id=coach_id
        )

    async def find_coach(self, conn, *, source: str | Source, external_id: str) -> Optional[int]:
        return await self.find(conn, entity="coach", source=source, external_id=external_id)

    async def ensure_country(self, conn, *, source: str | Source, external_id: str, country_id: int) -> int:
        return await self.ensure(
            conn, entity="country", source=source, external_id=external_id, internal_id=country_id
        )

    async def find_country(self, conn, *, source: str | Source, external_id: str) -> Optional[int]:
        return await self.find(conn, entity="country", source=source, external_id=external_id)

    async def ensure_city(self, conn, *, source: str | Source, external_id: str, city_id: int) -> int:
        return await self.ensure(
            conn, entity="city", source=source, external_id=external_id, internal_id=city_id
        )

    async def find_city(self, conn, *, source: str | Source, external_id: str) -> Optional[int]:
