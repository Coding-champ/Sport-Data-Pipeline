...
def process_one(args):
    if args.api_url:
        ...
        dt = perf_counter() - t0
        if args.verbose:
            print(f"Fetched WhoScored JSON in {dt*1000:.0f} ms -> {args.api_url}")
        print(
            json.dumps(
                {
                    "source": "whoscored",
    ...
    dt = perf_counter() - t0
    if args.verbose:
        print(f"Fetched WhoScored page in {dt*1000:.0f} ms -> {args.url}")
    print(
        json.dumps(
            {
                "source": "whoscored",
...
def main():
    p = argparse.ArgumentParser(description="WhoScored Collector (skeleton)")
    p.add_argument("--url", type=str, default=None, help="WhoScored page URL (fallback)")
    ...
    p.add_argument(
        ...
        type=str,
        default=".match-centre,.player-table,.team-header",
